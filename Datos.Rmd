...
4. Dado un individuo que no gana mas de 50k que podri tener condiciones para ganarlo, ¿que recomendacion podemos dar a esa persona para que gane mas de 50k?

5. ¿Es proporcional el nivel de educacion junto con la profesion que se ocupa a la cantidad de ingreso que genera?

6. Teniendo en cuenta que el PCA solo tiene en cuenta variables que son cuantitavivas, ¿es buena idea considerar este modelo como un candidato a mejor modelo predictivo?
...
10. ¿Cual es el mejor kernel para la separacion de nuestros datos?
...
PREGUNTAR
no se que distribucion siguen si es q siguen alguna hours parece una normal.
...
PREGUNTAR donde va esto
En la predicción para los datos del test nos daba un error de que había un valor que no existía, esto era porque en los datos test hay 2 observaciones en las que la variable education toman el valor de Preschool. Pero, estas son las dos únicas observaciones de la muestra, luego en los datos pertenecientes al train no existen observaciones donde se tomen estos valores, luego al crear el árbol, éste no tiene en cuenta dicha categoría, luego al después metersela para que prediga da un error.
