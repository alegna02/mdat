...
```{r}
biplot(prcomp(Cuantitativo_DatosTrain,scale=T))
```
...
A continuación, ajustaremos los parametros y obtendremos la prediccion definitiva.
...
```{r}
# Tasa de error pred1Ajustado
(12+72)/387
```

```{r}
# Precision pred1Ajustado
22/(12+22)
```

```{r}
# Tasa de error pred2Ajustado
(8+72)/387
```

```{r}
# Precision pred2Ajustado
22/(8+22)
```
...
parametros_svm <- expand.grid(C = c(0.1, 1, 10), gamma = c(0.1, 1, 10))
