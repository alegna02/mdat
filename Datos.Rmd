...
library(factoextra)
fviz_cluster(clusters7.datos, data = Cuantitativo)
...
Datos_Train2 <- rbind(Datos_Train2, Datos_Test2[c('209'),])
...
datos2.rf <- randomForest(x.train[,1:14], as.factor(x.train[,15]), ntree=100, importance=FALSE, proximity=TRUE, mtry=4, replace=FALSE)
y.pred.test.rf=predict(datos2.rf, x.test[,-c(15)])
table.libreria.test=table(y.pred.test.rf,  as.factor(x.test[,15]))
y.pred.train.rf= predict(datos2.rf, x.train[,-c(15)])
table.libreria.train=table(y.pred.train.rf,  as.factor(x.train[,15]))
print(table.libreria.test)
print(table.libreria.train)


datoNuevo <- data.frame(Age = 35, Workclass = "Private", fnlwgt = 125467, Education = "Bachelors", Education.num = 13, Marital.status = "Divorced", Occupation. = "Sales", Relationship = "Husband", Race = "White", Sex = "Female", Capital.gain = 1500, Capital.loss = 2000, Hours.per.week = 40, Native.country = "United-States")

datoNuevo <- datos[1,]

prediccionDatoNuevo=predict(datos2.rf, datoNuevo)


# Preguntar a ruben que cambios estan haciendo para que les mejore la tabla de predicciones
