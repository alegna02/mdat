...
6. ¿Cuál es la forma optima de agrupar las observaciones en función de sus características?

7. ¿Cuál de los modelos predictivos evaluados proporciona una mayor tasa de aciertos o exactitud en sus predicciones?
...
```
Con este gráfico podemos ver los histogramas de cada variables, con lo que podríamos ver que distribución sigue cada variable cuantitativa. Sin embargo, viendo los resultados obtenidos no podemos sacar ninguna conclusión sobre la distribución de los datos. Los únicos datos que parecen que se aproximan a una normal son las horas.
...
```{r}
boxplot(Cuantitativo_Datos$Age, main = "Boxplot de Edad", xlab = "", ylab = "Edad")
boxplot(Cuantitativo_Datos$FNLWGT, main = "Boxplot de FNLWGT", xlab = "", ylab = "FNLWGT")
```
En los gráficos podemos obervar que los valores que nos proporcionan de mediana, media, máximos, mínimos, etc coinciden con los observados anteriormente. Además, lo que podemos observar, es que tenemos bastantes valores atípicos, y esto se encuentran por encima del máximo, pero no por debajo del mínimo. Además, observando la variabilidad, vemos que la edad tiene bastante variabilidad pero sin embargo en la variable FNLWGT no existe tanta. Por último, en cuanto a la distribución de los datos, como la mediana en ambos casos se encuentra aproximadamente en la mitad de la caja, esto significa que los datos se aproximan a una distribución normal.

# Conclusiones Análisis exploratorio de datos

Como primera conclusión de esta parte podemos sacar que, los datos cuantitativos no están relacionados entre si, es decir, no existe correlación.

Además, hemos hecho una primera comprobación de que no hay indicios de que exista sesgo en cuanto al sexo ya que hemos visto que según los datos que tenemos el 86% de la mujeres cobra menos de 50000 y para hombres este porcentaje es del 70%, luego la diferencia es de un 16% y no es razonable para afirmar que exista sesgo, de igual manera, esto lo volveremos a comprobar más tarde.

Por último, hemos contestado una de las preguntas iniciales, y hemos visto que si influye tanto el nivel de educación, como el trabajo de la persona para los ingresos que se obtengan, cabe destacar, que hemos visto que las personas que tienen unos ingresos superiores a 50000 dólares suelen ser personas con estudios más avanzados, trabajos menos duros físicamente y con una edad cercana a los 40 entre otras cosas observadas.
...
Vemos las comparaciones mas importantes entre nuestras componentes principales:

```{r}
autoplot(prcomp(Cuantitativo_DatosTrain,scale=T),x=1,y=2,data = Cuantitativo_DatosTrain,loadings= TRUE,loadings.colour= 'blue',loadings.label.colour= 'red',loadings.label=TRUE, loadings.label.size= 4)

autoplot(prcomp(Cuantitativo_DatosTrain,scale=T),x=2,y=4,data = Cuantitativo_DatosTrain,loadings= TRUE,loadings.colour= 'blue',loadings.label.colour= 'red',loadings.label=TRUE, loadings.label.size= 4)

autoplot(prcomp(Cuantitativo_DatosTrain,scale=T),x=2,y=5,data = Cuantitativo_DatosTrain,loadings= TRUE,loadings.colour= 'blue',loadings.label.colour= 'red',loadings.label=TRUE, loadings.label.size= 4)

autoplot(prcomp(Cuantitativo_DatosTrain,scale=T),x=4,y=5,data = Cuantitativo_DatosTrain,loadings= TRUE,loadings.colour= 'blue',loadings.label.colour= 'red',loadings.label=TRUE, loadings.label.size= 4)
```
...
```{r}
pca1_pca4<-prcomp(Cuantitativo_DatosTrain,scale=T)$x[,c(1,4)]
plot(pca1_pca4)
```
...
## Conclusiones: Componentes Principales

Las conclusiones a las que llegamos es que este modelo utilizando componentes principales no es muy bueno a la hora de explicar, tanto la variabilidad de nuestros datos, como al hacer predicciones.

Esto se debe a que, como sólo empleamos variables cuantitativas (discretas y continuas) para la realización de este modelo, nos estamos dejando mucha variabilidad explicada en las otras variables categóricas que no entran en el modelo.

En nuestra base de datos, la mayoría de nuestras variables son categóricas por lo tanto es razonable pensar que la importancia y la significatividad de estas, sean importantes a la hora de realizar predicciones o contrastes de hipóstesis para nuestros datos.
...
Como los gráficos pairs anteriores no nos proporcionan una gran visibilidad de los clusters decidimos hacer el análisis cluster con PCA.
...
El cluster de abajo a la derecha reúne las observaciones que tienen un CapitalGain , edad y horas altas y CapitalLoss bajo.

El otro cluster extremo sería el de arriba cuyas observaciones se caracterizan por tener un CapitalLoss alto y valores bajos del resto de variables. A continuación, vemos cuales son los centroides de cada cluster. 
Por último, representamos los clusters dibujando las variables dos a dos.
...
# Conclusiones del análisis cluster

El número óptimo de clusters para la agrupación de nuestros datos es 4 ya que es aquel que permite una diferenciación mas efectiva de los datos.
...
-	Personas que de verdad cobran <=50K y el modelo lo ha predicho bien (<=50K) = 277
-	Personas que de verdad cobran >50K y el modelo lo ha predicho mal (<=50K) = 83
-	Personas que de verdad cobran <=50K y el modelo lo ha predicho mal (>50K) = 16
-	Personas que de verdad cobran >50K y el modelo lo ha predicho bien (>50K) = 11
...
Las conclusiones a las que podemos llegar usando este modelo es que no tiene tanta exactitud como otros modelos realizados, ya que tiene una exactitud de 74.42%, luego es peor en cuanto a las predicciones. Sin embargo, este modelo es bastante preciso con las predicciones, con un porcentaje de un 83.83%. Pero aún teniendo una precisión considerablemente alta, otros modelos siguen siendo mejores que el KNN en general.
...
Estos datos nos muestra que el modelo de Árboles de deisión tiene una exactitud del 80.62% (TP+TN)/N. Que es un valor bastante alto, luego el modelo es muy exacto en su predicción.
...
En nuestro caso podemos observar como para los datos de nuestro conjunto de test, el error que se comete no supera el 20%. Teniendo en cuenta la dimensión de nuestros datos junto con la preción y exctitud que tienen podemos decir que no es un mal modelo para predecir.

De igual manera, vamos a seguir investigando modelos, algunos más complejos que este, que nos permitan bajar ese error que cometemos y aumentar el valor de la preción y de la exactitud.

No obstante, si lo que queremos es explicar de la manera mas sencilla y breve lo que sucede con el ingreso de las personas, por el hecho de  tratarse de un modelo "sencillo" y muy entendible por cualquiera (muy parecido a un juego), nos sirve para tener de cierta intuición de lo que le sucede a nuestros datos.
...
Datos_con_Factores.rf

```
En el OOB obtenemos la matriz de confusión, y vemos las predicciones que hace para los datos del train. Para hacerlo lo que hace es hacer el modelo sin el dato a predecir y luego le predice, y esto lo hace con todos los datos. Los valores que obtenemos de esta matriz son los siguientes, 

-	Personas que de verdad cobran <=50K y el modelo lo ha predicho bien (<=50K) = 536

-	Personas que de verdad cobran >50K y el modelo lo ha predicho mal (<=50K) = 49

-	Personas que de verdad cobran <=50K y el modelo lo ha predicho mal (>50K) = 75

-	Personas que de verdad cobran >50K y el modelo lo ha predicho bien (>50K) = 116

Con estos datos el random forest tiene una exactitud del 84.02% (TP+TN)/N. Que es un valor bastante alto, luego el modelo es muy exacto en su predicción.

```{r}
(536+116)/776
```

De manera recíproca tiene un error del 15.97% (FP+FN)/N

```{r}
(49+75)/776
```

Además, tiene una precisión del 60.73% TP/(TP+FP). Que no es muy alta pero está bien, luego el modelo no es muy preciso con las predicciones.

```{r}
116/(116+75)
```

Si hicieramos una predicción con los datos del train, los haría todos bien, esto es normal ya que ha sido entrenado con estos datos y los árboles han sido creados a través de estos, pero como hemos visto antes, esto no es una forma de hacer las matrices de confusión en random Forest.
...
Como podemos ver en la matriz de confusión que sale del modelo, esta tiene buenos valores en cuanto a exactitud aunque peores en cuando a precisión, de igual manera, de manera global, este es el mejor de los modelos que tenemos para predecir nuestros datos. 

Además, en este modelo hemos respondido a una de las preguntas iniciales, si existía sesgo en cuanto al sexo. Tras analizar si existía, usando este modelo, hemos visto que no existe ningún sesgo en el sexo, ya que las predicciones, usando la variable sexo o sin usarla salen practicamente las mismas, luego no predice que vayas a tener más ingresos teniendo en cuenta el sexo.

#SVM

Las máquinas de vectores soporte o Support Vector Machines son otra técnica de clasificado de elementos de una base de datos.

Intuitivamente, lo que este método intenta es encontrar un hiperplano separador para el espacio donde estén nuestros datos. Esto lo consigue tomando un subconjunto de los mismos (en un espacio de dimensionalidad alta empleando un kernel, algo matemáticamente muy técnico pero riguroso) lo suficientemente amplio para no obtener mucho error en la clasificación, pero a la vez lo suficientemente pequeño como para no ser muy complejo y con dicho subconjunto intentar dibujar el hiperplano.

Este hiperplano puede ser lineal, radial, polinomial, etc; dependiendo del kernel que se utilice en la transformación de los puntos al espacio de dimension alta.
...
# Predecimos para sv2Ajustado
pred2Ajustado = predict(svm2Ajustado, Cuantitativo_DatosTest)
pred2Ajustado.prob = predict(svm2Ajustado.prob, Cuantitativo_DatosTest, prob = T)
...
El SVM que nos proporciona los mejores resultados es aquel realizado con los parámetros ajustados y un kernel polinomial. Siendo un buen modelo predictivo, no es el mejor ya que hemos observado anteriormente que Random Forest proporciona mejores resultados.

# Curvas ROC

Las curvas ROC nos permiten ver cual es mejor modelo, el cual utilizaremos a la hora de predecir con los datos de validacion. Para ello:
...
Como se puede observar el mejor modelo, y el que se va a emplear para estudiar el conjunto de validación es `RANDOM FOREST`.
...
#Definimos los parametros que vamos a estudiar
parametros_svm <- expand.grid(C = c(0.3, 0.1, 0.2), gamma = c(0.3, 0.1, 0.2))

entrenar_evaluar_svm <- function(C, gamma, x.train, x.validation) {
  modelo_Final <- svm(Income. ~ ., data = x.train, kernel = "radial", cost = C, gamma = gamma)
  predicciones_Finales <- predict(modelo_Final, x.validation[, -15])  
  confusion_matrix <- table(predicciones_Finales, x.validation$Income.)
  precision <- sum(diag(confusion_matrix)) / sum(confusion_matrix)
  return(1 - precision)
}

modelo_tune <- tune(svm, Income. ~ ., data = x.train, kernel = "radial",
                    ranges = parametros_svm, tunecontrol = tune.control(sampling = "cross"))

# Mostramos los mejores parametros que hemos encontrado

mejores_parametros <- modelo_tune$best.parameters
print(mejores_parametros)

modelo_definitivo <- svm(Income. ~ ., data = x.train, kernel = "radial",
                    cost = mejores_parametros$C, gamma = mejores_parametros$gamma)
```

## Predicciones

```{r}
y.pred.validation.svm=predict(modelo_definitivo, x.validation[,-c(15)])
table.libreria.validation=table(y.pred.validation.svm,  as.factor(x.validation[,15]))
...
```
```{r}
# Tasa de error y.pred.validation.svm
(0+112)/387
```
...
print(confusionMatrix(data = y.pred.validation.svm,reference=as.factor(x.validation$Income.)))
```
...
```
## Conclusiones: Datos Validación

Una vez realizado el estudio sobre los datos de validación, podemos responder a una de lsa preguntas más importantes que nos hicimos al principio de este proyecto y es la de si todas las variables eran igual de significativas.

Gracias al Random Forest, como modelo escogido, y gracias a los datos de validación y su estudio profundo, podemos observar como las variables más importantes de nuestros datos son: `Age`, `Occupation`,`fnlwgt`, `Education`, `Relationship` y `Capital.gain`.

Est tiene sentido, puesto que la intuición nos dice que para determinar si una persona gana más o menos de 50K la edad es un rasgo importante, la educación que ha conseguido esa persona etc.
...
A su vez, todas las preguntas que hemos realizado al comienzo del trabajo han podido ser contestadas de forma convincente basandonos en datos y en resultado que hemos obtenido.
...
Entonces nos planteamos la siguiente pregunta, ¿que ocurriria si eliminasemos esa variable de nuestro conjunto de variables?
